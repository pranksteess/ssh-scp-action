...
executeSSH() {
  ...
  echo "ssh -o StrictHostKeyChecking=no -p ${INPUT_BOARD_PORT:-22} $INPUT_BOARD_USER@$INPUT_BOARD_HOST 'chsh -s /bin/bash;/bin/bash $INPUT_BOARD_SCRIPT $INPUT_DST_USER $INPUT_DST_HOST $INPUT_FILE_NAME $INPUT_DST_FILE_PATH \"$COMMANDS\"'"
  ...
  ssh -o StrictHostKeyChecking=no -p ${INPUT_BOARD_PORT:-22} $INPUT_BOARD_USER@$INPUT_BOARD_HOST 'chsh -s /bin/bash;/bin/bash $INPUT_BOARD_SCRIPT $INPUT_DST_USER $INPUT_DST_HOST $INPUT_FILE_NAME $INPUT_DST_FILE_PATH "$COMMANDS"'
}
